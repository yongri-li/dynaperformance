      ...
      value: function _createPhotoSwipeInstance(items, selectedImageIndex) {
        ...
        this.photoSwipeInstance = new PhotoSwipe(photoswipeContainer, false, items, {
          ...
          getThumbBoundsFn: function getThumbBoundsFn(index) {
            var index1 = $(".pro_main_slider .slick-active").data('slick-index');
            //var thumbnail = document.querySelector('.pro_main_slider .slick-active').querySelector('[data-slick-index="' + parseInt(index) + '"] img'),
            //var thumbnail = $('.pro_main_slider .slick-active').find('[data-slick-index="' + parseInt(newIndex) + '"] img'),
            var thumbnail = $(".pro_main_slider").find(`[data-slick-index='${index1}'] img`),
...
//           if ( $('.hulkapps_submit_cart').length > 0 ) { 
//             $("div[id*='hulkapps_option_list_']").closest("form").find(':submit').removeClass('hulkapps_submit_cart');        
//           }
//         }, 1000);
//       }
...
//         this.documentDelegate.on('cart:refresh', this._rerenderCart.bind(this, false));
        document.addEventListener('cart:refresh', this._rerenderCart(this, false));
      }
    }, {
      key: '_updateCartNote',
          ...
          if (_this35.options['drawer'] && elementToAnimate) {
            var timelineLite = new TimelineLite({ onComplete: function onComplete() {
                content.text().then(function (html) {
                  ...
                  hulkProductOption.hulkAjaxCart();
                });
              } });
    ...
    _createClass(ProductSection, [{
      ...
      value: function _updateSlideshowImage(event) {
        ...
        /*
         ...
         $('.product_thumbnails img').each(function(){
            
            if($(this).attr("data-image-id") == variant.featured_image.id)
            {
                //console.log("left=" + $(this).attr("data-image-id"));
            	//console.log("right=" + variant.featured_image.id);
                //alert("yes");
              
              	$(this).trigger("click");
              
              	//$(this).parent().parent().parent().parent(".slick-slide").trigger("click");
              
              	//var slideIndex = $(this).parent().parent().parent().parent(".slick-slide").attr("data-slick-index");
        ...
        */
        ...
        $("[data-image-id="+variant.featured_image.id+"]").closest('.product_thumbnails').trigger('click');
...
$('.customization-button').on('click', function(){
  $('.hulkapps_custom_options').show();
  $([document.documentElement, document.body]).animate({
        scrollTop: $(".hulkapps_custom_options").offset().top - $('#section-header').height()
    }, 500);
});

$('.home-hero-slideshow .ButtonGroup__Item').on('click', function(e){
  e.preventDefault();
  $([document.documentElement, document.body]).animate({
    scrollTop: $(".select-your-section").offset().top
  }, 1000);
})

$(window).scroll(function(){
  var steeringAddToCart = $('.steering-addtocart').length;
  if(steeringAddToCart){
    var windowScrollTop = $(window).scrollTop();
    var addToCartButtonScrollTop = $('.steering-addtocart').offset().top - $('.Header__Wrapper').height();
    if(windowScrollTop > addToCartButtonScrollTop){
      $('.steering-addtocart button').addClass('sticky');
    } else {
      $('.steering-addtocart button').removeClass('sticky');
    }
  }
});
